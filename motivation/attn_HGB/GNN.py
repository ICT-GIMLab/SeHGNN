...
class myGAT2(nn.Module):
    ...
    def forward(self, features_list, e_feat, average_na_layers=[], average_sa_layers=[], save=False):
        h = []
        for fc, feature in zip(self.fc_list, features_list):
            h.append(fc(feature))
        h = torch.cat(h, 0)
        ...
        for l in range(self.num_layers):
            h, res_attn = self.gat_layers[l](self.g, h, e_feat, res_attn=res_attn,
                                             average_na=l in average_na_layers, average_sa=l in average_sa_layers, save=save)
            h = h.flatten(1)
        logits, _ = self.gat_layers[-1](self.g, h, e_feat, res_attn=None,
                                        average_na=self.num_layers in average_na_layers, average_sa=l in average_sa_layers, save=save)
        logits = logits.mean(1)
        return logits
